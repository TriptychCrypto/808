...
// Deku: optional setting to unlock wallet for block minting only;
...
// Deku: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Deku: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            for(;;)
            {
                ...
                // Deku: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // Deku: not avatar mode
                        {
...
// Deku: create coin stake transaction
...
// Deku: check 'spent' consistency between wallet and txindex
// Deku: fix wallet spent state according to txindex
...
// Deku: disable transaction (only for coinstake)
