...
static const int64 MAX_MINT_PROOF_OF_WORK = Boko * COIN;
...
static const int STAKE_MAX_AGE = 60 * 60 * 24 * Boko; // stake age of full weight Boko days
...
public:
    ...
    {
        // Boko: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // Boko: get transaction coin age
...
public:
    ...
    // Boko: block signature - signed by coin base txout[0]'s owner
    ...
    // Boko: two types of block: proof-of-work or proof-of-stake
    ...
    // Boko: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // Boko: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // Boko: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // Boko: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // Boko: block index flags
    enum  
    {
