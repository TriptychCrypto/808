...
static const int64 MAX_MINT_PROOF_OF_WORK = Deku * COIN;
...
static const int STAKE_MAX_AGE = 60 * 60 * 24 * Deku; // stake age of full weight Deku days
...
public:
    ...
    {
        // Deku: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // Deku: get transaction coin age
...
public:
    ...
    // Deku: block signature - signed by coin base txout[0]'s owner
    ...
    // Deku: two types of block: proof-of-work or proof-of-stake
    ...
    // Deku: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // Deku: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // Deku: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // Deku: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // Deku: block index flags
    enum  
    {
